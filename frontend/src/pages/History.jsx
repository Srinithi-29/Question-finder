    ...
    <div className="min-h-[calc(100vh-4rem)] bg-slate-50 text-slate-800 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-10 pb-6 border-b border-slate-200">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Question History</h1>
            <p className="text-slate-600 text-sm mt-1">Review, search, and filter your previously asked study questions</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="md:col-span-2 relative">
            ...
            <input
              ...
              className="w-full bg-white border border-slate-200 focus:border-brand-500 rounded-xl py-3.5 pl-11 pr-4 text-slate-900 placeholder-slate-400 focus:outline-none transition-all text-sm shadow-sm"
              value={searchTerm}
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full bg-white border border-slate-200 focus:border-brand-500 rounded-xl py-3.5 pl-11 pr-4 text-slate-900 focus:outline-none transition-all text-sm appearance-none cursor-pointer shadow-sm"
            >
              {tags.map((tag) => (
        ...
        {error && (
          <div className="mb-6 flex items-start gap-2.5 p-4 rounded-xl bg-red-55 border border-red-200/60 text-red-750 text-sm">
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-md">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-500">
              <Loader2 className="h-10 w-10 animate-spin text-brand-650 mb-4" />
                      ...
                      <React.Fragment key={q.id}>
                        ...
                        {isExpanded && (
                          <tr className="bg-slate-50/50">
                            <td colSpan="4" className="py-4 px-8 border-t border-slate-100">
                                ...
                                {matchesCount > 0 ? (
                                  <div className="grid gap-2.5">
                                    {q.similarQuestions.map((match, idx) => (
                                      <div
                                        key={idx}
                                        className="flex items-center justify-between gap-4 p-3 rounded-xl bg-white border border-slate-200/80 shadow-sm"
                                      >
                                        <span className="text-xs text-slate-700 italic">
                                          "{match.question}"
                                        </span>
                                        <span className="shrink-0 text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full font-mono">
